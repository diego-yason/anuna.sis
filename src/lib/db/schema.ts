...
export const terms = pgTable("terms", {
    term: smallint().primaryKey().generatedByDefaultAsIdentity({
        name: "terms_term_seq",
        startWith: 1,
        increment: 1,
        minValue: 1,
...
export const courseEquivalences = pgTable(
    ...
    (table) => [
        ...
        foreignKey({
            ...
            .onDelete("cascade"),
        primaryKey({
            columns: [table.courseA, table.courseB],
            name: "courseEquivalences_pkey",
        }),
    ]
);
...
export const curriculums = pgTable(
    ...
    (table) => [
        ...
        foreignKey({
            ...
            .onDelete("cascade"),
        primaryKey({
            columns: [table.courseCode, table.program],
            name: "curriculums_pkey",
        }),
    ]
);

export const personalSchedules = pgTable(
    "personalSchedules",
    {
    ...
    (table) => [
        foreignKey({
            ...
            .onDelete("cascade"),
        primaryKey({
            columns: [table.schedule, table.classNumber],
            name: "personalSchedules_pkey",
        }),
    ]
);

export const courseRequisites = pgTable(
    "courseRequisites",
    {
    ...
    (table) => [
        ...
        foreignKey({
            ...
            .onDelete("cascade"),
        primaryKey({
            columns: [table.requisite, table.incoming],
            name: "courseRequisites_pkey",
        }),
    ]
);

export const sectionSchedules = pgTable(
    "sectionSchedules",
    {
        // You can use { mode: "bigint" } if numbers are exceeding js number limitations
        classNumber: bigint({ mode: "number" }).notNull(),
        term: smallint().notNull(),
        day: varchar().array().notNull(),
        start: time(),
        end: time(),
        room: text(),
    },
    (table) => [
        foreignKey({
            ...
            .onDelete("cascade"),
        primaryKey({
            columns: [table.classNumber, table.term, table.day],
            name: "sectionSchedules_pkey",
        }),
    ]
);
